...
export default function DonationModal({ onClose }: DonationModalProps) {
    return (
        <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-sm max-h-[85vh] flex flex-col rounded-3xl shadow-2xl relative overflow-hidden text-center animate-in zoom-in-95 duration-300 ease-out">
                {/* Scrollable Body */}
                <div className="overflow-y-auto p-8 custom-scrollbar">
                    <button onClick={onClose} className="absolute top-4 right-4 text-slate-300 hover:text-slate-500 transition-colors z-10 cursor-pointer">
                        <XCircle size={24} />
                    </button>

                    <div className="w-16 h-16 bg-yellow-100 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce shrink-0">
                    ...
                    <button onClick={onClose} className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl font-bold text-sm transition-colors cursor-pointer">
                        Close
                    </button>
