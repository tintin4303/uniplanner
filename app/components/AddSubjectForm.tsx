    ...
    <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-200 mb-6 animate-in fade-in zoom-in-95 duration-200 ease-out">
      ...
      <div className="space-y-6">
                    ...
                    <div key={cIdx} className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                      <div className="flex flex-wrap gap-2 mb-3 overflow-x-auto pb-1">
                        ...
                        {TIME_PRESETS.map((preset) => (
                          <button key={preset.label} onClick={() => applyPreset(sec.id, cIdx, preset.start, preset.end)} className="px-3 py-1 bg-slate-100 hover:bg-indigo-100 text-slate-600 hover:text-indigo-700 text-[10px] font-bold rounded-full transition-colors whitespace-nowrap cursor-pointer">{preset.label}</button>
                        ))}
                      </div>
                      <div className="flex flex-col gap-2">
                        <select className="p-2 rounded-lg text-xs border border-slate-200 bg-white min-w-[120px]" value={cls.day} onChange={e => updateClass(sec.id, cIdx, 'day', e.target.value)}>{DAYS.map(d => <option key={d} value={d}>{d}</option>)}</select>
                        <div className="flex gap-2 items-center">
                          <input type="time" className={`p-2 rounded-lg text-xs border bg-white w-[110px] min-w-0 ${isSuspiciousTime(cls.start) ? 'border-red-400 text-red-500 font-bold' : 'border-slate-200'}`} value={cls.start} onChange={e => updateClass(sec.id, cIdx, 'start', e.target.value)} />
                          <span className="text-slate-400 shrink-0">-</span>
                          <input type="time" className="p-2 rounded-lg text-xs border border-slate-200 bg-white w-[110px] min-w-0" value={cls.end} onChange={e => updateClass(sec.id, cIdx, 'end', e.target.value)} />
                        </div>
        ...
        <div className="flex gap-3">
          <button onClick={addSection} className="flex-1 py-3 border-2 border-dashed border-slate-300 text-slate-400 rounded-xl font-bold hover:border-indigo-400 hover:text-indigo-500 transition-colors cursor-pointer">Add Option</button>
